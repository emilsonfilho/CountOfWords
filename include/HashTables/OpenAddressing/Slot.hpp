 ...
 * Parameterized constructor that initializes the slot with a given key and
 * value, and sets the status to ACTIVE.
 *
 * @note The `Key` and `Value` types, as well as the `Status` enumeration, are
