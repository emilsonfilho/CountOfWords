 ...
 *
 * The AVLTree class implements a self-balancing binary search tree that
 * maintains the AVL property. It supports operations such as insertion,
 * deletion, and search while ensuring logarithmic time complexity.
 *
...
private:
   ...
   *
   * The balance factor is the difference between the height of the right
   * subtree and the height of the left subtree.
   *
   * @param node A pointer to the node for which the balance factor is
   * calculated.
   ...
   *
   * This function performs rotations to restore the AVL tree's balance if
   * necessary.
   *
   ...
   *
   * This function removes the smallest node in the right subtree during
   * deletion.
   *
   ...
   */
  AVLNode<Key, Value> *removeSuccessor(AVLNode<Key, Value> *root,
   ...
   */
  AVLNode<Key, Value> *insert(const Key &key, const Value &value,
                              AVLNode<Key, Value> *node);
   ...
   */
  AVLNode<Key, Value> *update(const Key &key, const Value &value,
                              AVLNode<Key, Value> *node);
   ...
   * @param outValue A reference to a pointer that will store the address of the
   * value associated with the key.
   * @return A pointer to the balanced node after insertion or update.
   * @details This method handles both insertion of new nodes and updating
   * existing node values. It recursively traverses the tree to find the correct
   * position for the key. If the key is found, `outValue` is set to point to
   * the existing value. If a new node is created, `outValue` points to its new
   * value. It also updates `maxKeyLen` and `maxValLen` based on the display
   * size of the key and value, and performs AVL tree rotations via `fixupNode`
   * to maintain balance.
   */
  AVLNode<Key, Value> *upsert(const Key &key, AVLNode<Key, Value> *node,
                              Value *&outValue);
...
public:
  ...
  /**
   * @brief Searches for a key in the AVL tree and retrieves its associated
   * value.
   *
   * @param key The key to search for.
   * @param outValue A reference to store the associated value if the key is
   * found.
  ...
  /**
   * @brief Retrieves the count of comparisons made during operations on the AVL
   * tree.
   *
   ...
   * collect metrics or perform operations on the AVLTree.
   *
