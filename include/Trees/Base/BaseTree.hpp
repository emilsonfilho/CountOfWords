...
class BaseTree {
  ...
  /**
   * @brief Resets the internal counter used for tracking operations or metrics
   * in the tree.
   *
   * This function delegates the reset operation to the derived class by calling
   * its `resetCounter` method. It ensures that the counter is cleared and ready
   * for reuse.
   */
  void clearCounter();
...
protected:
  ...
  /**
   * @brief Represents the maximum length of a key that can be stored in the
   * tree.
   */
  size_t maxKeyLen;
   ...
   * length, and the rotations count to zero. Additionally, it clears the
   * counter used for tracking operations.
   */
  BaseTree(Node *r);
  ...
  /**
   * @brief Recursively clears (deletes) nodes in a subtree, avoiding a
   * comparison node.
   * @details This method is typically used in destructors to deallocate tree
   * nodes.
   * @param node The current node to clear.
   * @param comp A comparison node (e.g., a sentinel or a node not to be
   * deleted).
   */
   ...
   *        except for the comparison node, and sets the root to the default
   * root.
   *
   * @param node The starting node to clear.
   * @param comp The comparison node that will not be cleared. Defaults to
   * nullptr.
   * @param defaultRoot The new root node to set after clearing. Defaults to
   * nullptr.
   */
  void reset(Node *node, Node *comp = nullptr, Node *defaultRoot = nullptr);

  /**
   * @brief Performs an in-order traversal of the subtree and prints node
...
// methods. This must be at the end of the header file.
