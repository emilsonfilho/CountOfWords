 ...
 *
 * The RedBlackTree class implements a self-balancing binary search tree that
 * maintains the Red-Black Tree properties. It supports operations such as
 * insertion, deletion, and search while ensuring logarithmic time complexity.
 *
...
private:
  ...
  static constexpr RedBlackNode<Key, Value> *const NIL =
      &NIL_NODE; ///< Sentinel node representing null leaves.
   ...
   *
   * This function restores the Red-Black Tree properties by performing
   * rotations and recoloring nodes as necessary.
   *
   ...
   *
   * This function ensures that the Red-Black Tree remains balanced and
   * maintains its properties (e.g., no two consecutive red nodes, equal black
   * height for all paths) after a node has been deleted. It performs rotations
   * and recoloring as necessary.
   *
   * @param x A pointer to the node that replaces the deleted node, or NIL if no
   * replacement exists.
   *
   * @note This function assumes that the tree is a valid Red-Black Tree before
   * the deletion and that the node being fixed up is initially black.
   */
   ...
   * the tree remains balanced and adheres to the Red-Black Tree rules by
   * calling the deleteFixup function when necessary.
   *
...
public:
  ...
  /**
   * @brief Searches for a key in the Red-Black Tree and retrieves its
   * associated value.
   *
  ...
  /**
   * @brief Retrieves the count of comparisons made during operations on the
   * Red-Black Tree.
   *
  ...
  /**
   * @brief Retrieves the total number of rotations performed by the Red-Black
   * Tree.
   *
   * This method returns the count of rotations (both left and right) that have
   * been executed during insertions or deletions to maintain the Red-Black Tree
   * properties.
   *
   ...
   * collect metrics or perform operations on the RedBlackTree.
   *
